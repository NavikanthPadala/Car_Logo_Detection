...
def detect_image(models, image, confidence):
    ...
    if (len(plate_boxes) > 0) and ("Plate" in models):
        for box in plate_boxes:
            ...
            cv2.rectangle(overlay_image, (int(x1), int(y2)), (int(x1) + text_size[0], int(y2) + 17), (222, 222, 255),
                          -1)
            # Add text
            cv2.putText(overlay_image, f"Plate {box.conf[0]:.2f}", (int(x1), int(y2) + 15), cv2.FONT_HERSHEY_DUPLEX,
                        0.5, (0, 0, 255), 1)

    if (len(car_boxes) > 0) and ("Car" in models):
        for box in car_boxes:
            if coco_names[int(box.cls)] == 'car' or coco_names[int(box.cls)] == 'truck' or coco_names[
                int(box.cls)] == 'bus':
                x1, y1, x2, y2 = [box.xyxy[0][i].item() for i in range(4)]
                cv2.rectangle(overlay_image, (int(x1), int(y1)), (int(x2), int(y2)), (202, 98, 0), 2)
                # Draw black background rectangle
                text_size, _ = cv2.getTextSize(f"{coco_names[int(box.cls)]} {box.conf[0]:.2f}", cv2.FONT_HERSHEY_DUPLEX,
                                               0.5, 1)
                cv2.rectangle(overlay_image, (int(x1), int(y1)), (int(x1) + text_size[0], int(y1) + 17),
                              (255, 219, 186), -1)
                # Add text
                cv2.putText(overlay_image, f"{coco_names[int(box.cls)]} {box.conf[0]:.2f}", (int(x1), int(y1) + 15),
                            cv2.FONT_HERSHEY_DUPLEX, 0.5, (202, 98, 0), 1)
...
def main():
    ...
    with col2:
        ...
        st.write('\n')
        st.markdown(
            "<h1><span style='color:#000000; font-size:45px; font-family:Monaco, Monospace;  font-style: italic;'>Vehicle Logo & Plate Detection</h1>",
    ...
    with tab2:
        st.markdown(
            "Our project aims to simplify the task of vehicle logo and Moroccan license plate recognition with cutting-edge AI technology.")
        st.markdown(
            " Whether you're a developer, a researcher, or simply curious about the capabilities of object detection, our platform offers a user-friendly interface to explore and utilize advanced models, allowing you to detect logos and license plates with precision and efficiency. Plus, you can input your data in various formats (images, videos, or live webcam streams) making the process seamless and adaptable to your workflow.")
        st.markdown(
            "Explore our app to uncover the potential of object detection and discover the extensive list of classes our models can identify below, ranging from popular vehicle logos to Moroccan license plate specific designs. Let's embark on this journey together and unlock the power of AI in recognizing objects in the world around us!")
        st.write('##')
    ...
    with tab1:
        ...
        if choice == 'Upload Image':
            ...
            if source_img:
                ...
                clicked = ui.button(text="Detect Objects", key="styled_btn_tailwind",
            ...
            if uploaded_video != None:
                ...
                if clicked:
                    ...
                    with col2:
                        ...
                        out = cv2.VideoWriter("detected_video.mp4", fourcc, float(frames_per_second),
                                              (720, int(720 * (9 / 16))))
                        st_frame = st.empty()
        ...
        # Case 3 : Use Webcam
            ...
            clicked = ui.button(text="Open Camera", key="styled_btn_tailwind",
                                className="bg-red-500 text-white font-bold")
            if clicked:
                # Loading camera
                ...
                while (cap.isOpened()):
                    ...
                    if success:
                        ...
                        cv2.putText(overlay_image, "FPS: " + str(round(fps, 2)), (10, 50), cv2.FONT_HERSHEY_PLAIN, 3,
                                    (0, 0, 0), 3)
